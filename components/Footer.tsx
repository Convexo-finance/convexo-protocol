...
export default async function Footer() {
  ...
  const technologyLinks = [
    { name: t('ourTechnology'),          href: '/technology' },
    { name: t('infrastructureProviders'), href: '/technology#providers' },
  ];

  const legalLinks = [
    { name: t('complianceHub'),    href: '/compliance' },
    { name: t('privacyPolicy'),    href: '/privacy' },
    { name: t('termsConditions'),  href: '/terms' },
    { name: t('amlCft'),           href: '/aml-cft' },
  ];

  const contactLinks = [
    { name: 'hello@convexo.xyz', href: 'mailto:hello@convexo.xyz', internal: false },
    { name: 'WhatsApp',          href: 'https://wa.me/573186766035', internal: false },
    { name: t('aboutUs'),        href: '/about', internal: true },
  ];
      ...
      <div className="max-w-[1600px] mx-auto px-8 md:px-12 py-20">
        <div className="grid grid-cols-2 md:grid-cols-12 gap-x-8 gap-y-12">
          ...
          <div className="md:col-span-2 flex flex-col gap-4">
            <span className="label-institutional text-primary">{t('productsTitle')}</span>
            {productLinks.map((l) => (
              <Link
                key={l.href}
                href={l.href}
          ...
          {/* Technology — 2 cols */}
          <div className="md:col-span-2 flex flex-col gap-4">
            <span className="label-institutional text-primary">{t('technologyTitle')}</span>
          ...
          {/* Legal — 2 cols */}
          <div className="md:col-span-2 flex flex-col gap-4">
            <span className="label-institutional text-primary">{t('legalTitle')}</span>
            {legalLinks.map((l) => (
              <Link
                key={l.href}
                href={l.href}
          ...
          {/* Connect + Verification — 3 cols */}
          <div className="md:col-span-3 flex flex-col gap-8">
            {/* Connect */}
            <div className="flex flex-col gap-4">
              <span className="label-institutional text-primary">{t('connectTitle')}</span>
              {contactLinks.map((l) =>
                l.internal ? (
                  <Link
                    key={l.name}
            ...
            {/* Verification badges */}
            <div className="flex flex-col gap-4">
              <span className="label-institutional text-primary">{t('verificationTitle')}</span>
        ...
        <div className="max-w-[1600px] mx-auto px-8 md:px-12 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              ...
              <span className="label-institutional text-on-surface/30">{t('liveNetwork')}</span>
            </div>
            <Link
              href="/compliance"
